...
WWPrintDeviceProp::~WWPrintDeviceProp(void)
{
    for (auto ite = mConnSet.begin(); ite != mConnSet.end(); ++ite) {
        auto r = *ite;
        SafeRelease(&r);
    }
    mConnSet.clear();

    for (auto ite = mTopoSet.begin(); ite != mTopoSet.end(); ++ite) {
        auto r = *ite;
        SafeRelease(&r);
    }
    mTopoSet.clear();
...
HRESULT
WWPrintDeviceProp::PrintDeviceProperties(IMMDevice *device)
{
    HRESULT hr = S_OK;
    IAudioFormatEnumerator *afEnum = nullptr;
    UINT32 afCount = 0;
    ISpatialAudioClient *saClient = nullptr;
    UINT32 maxDynObjCount = 0;
    ...
    HRG(device->Activate(
        __uuidof(ISpatialAudioClient), CLSCTX_INPROC_SERVER, &pv, (void**)&saClient));
    assert(saClient);

    HRG(saClient->GetMaxDynamicObjectCount(&maxDynObjCount));
    ...
    for (UINT32 i = 0; i < afCount; ++i) {
        ...
        if (wfex->wFormatTag != WAVE_FORMAT_EXTENSIBLE) {
            ...
            printf("  %d: %dHz %dbit %dch container=%dbit %s\n",
                ...
                WWGuidToKsDataFormatStr(&wfext->SubFormat));
        }
    }
...
HRESULT
WWPrintDeviceProp::PrintDeviceTopo(IMMDevice *device)
{
    HRESULT hr = S_OK;
...
HRESULT
WWPrintDeviceProp::PrintDeviceTopo1(int layer, IDeviceTopology *topo)
{
    HRESULT hr = S_OK;
    LPWSTR devIdStr = nullptr;
...
HRESULT
WWPrintDeviceProp::PrintConnector(int layer, IConnector *conn)
{
    HRESULT hr = S_OK;
...
HRESULT
WWPrintDeviceProp::PrintSubunit(int layer, ISubunit *su)
{
    HRESULT hr = S_OK;
    IPart *part = nullptr;
...
HRESULT
WWPrintDeviceProp::PrintPart(int layer, IPart *part)
{
    HRESULT hr = S_OK;
    PartType pt;
    LPWSTR name = nullptr;
...
HRESULT
WWPrintDeviceProp::PrintAudioMute(int layer, IAudioMute *am)
{
    HRESULT hr = S_OK;
    BOOL mute = FALSE;
...
HRESULT
WWPrintDeviceProp::PrintAudioVolumeLevel(int layer, IAudioVolumeLevel *avl)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
...
HRESULT
WWPrintDeviceProp::PrintAudioPeakMeter(int layer, IAudioPeakMeter *apm)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    float curL;
...
HRESULT
WWPrintDeviceProp::PrintAudioAutoGainControl(int layer, IAudioAutoGainControl *agc)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    BOOL bEnabled = FALSE;
...
HRESULT
WWPrintDeviceProp::PrintAudioBass(int layer, IAudioBass *ab)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    float minL, maxL, stepL, curL;
...
HRESULT
WWPrintDeviceProp::PrintAudioChannelConfig(int layer, IAudioChannelConfig *acc)
{
    HRESULT hr = S_OK;
    DWORD cc = 0;
...
HRESULT
WWPrintDeviceProp::PrintAudioInputSelector(int layer, IAudioInputSelector *ais)
{
    HRESULT hr = S_OK;
    UINT id = 0;
...
HRESULT
WWPrintDeviceProp::PrintAudioLoudness(int layer, IAudioLoudness *al)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    BOOL bEnabled = FALSE;
...
HRESULT
WWPrintDeviceProp::PrintAudioMidrange(int layer, IAudioMidrange *p)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    float minL, maxL, stepL, curL;
...
HRESULT
WWPrintDeviceProp::PrintAudioOutputSelector(int layer, IAudioOutputSelector *aos)
{
    HRESULT hr = S_OK;
    UINT id = 0;
...
HRESULT
WWPrintDeviceProp::PrintAudioTreble(int layer, IAudioTreble *p)
{
    HRESULT hr = S_OK;
    UINT nCh = 0;
    float minL, maxL, stepL, curL;
...
HRESULT
WWPrintDeviceProp::PrintKsJackDesc(int layer, IKsJackDescription *jd)
{
    HRESULT hr = S_OK;
    UINT n = 0;
...
HRESULT
WWPrintDeviceProp::PrintControlInterface(int layer, int id, IControlInterface *ci)
{
    HRESULT hr = S_OK;
    LPWSTR name = nullptr;
    GUID guid;
...
HRESULT
WWPrintDeviceProp::PrintKsFormatSupport(int layer, IKsFormatSupport *fs)
{
    HRESULT hr = S_OK;
