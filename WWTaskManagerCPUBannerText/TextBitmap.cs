...
namespace WWTaskManagerText {
    class TextBitmap {
        ...
        private static readonly ulong[] m8x8Fonts = new ulong[] {
            0x0000000000000000,   // U+0000
        ...
        public static byte[] Build(string text) {
            ...
            int imageW = rBA.Length * 8;
            int imageH = 8;

            var grayImage = new byte[imageW * imageH];

            for (int i = 0; i < rBA.Length; ++i) {
                ulong p = rBA[i];
                for (int y = 0; y < 8; ++y) {
                    for (int x = 0; x < 8; ++x) {
                        ulong mask = 1UL << (x + y*8);
                        bool b = (p & mask) != 0;
                        grayImage[x + i*8 + (7-y) * imageW] = (byte)(b ? 0 : 255);
                    }
                }
            }
